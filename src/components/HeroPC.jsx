...
export default function HeroPC() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.to(logoRef.current,  { opacity: 1, y: 0, filter: "blur(0px)", duration: 1.0 }, 0.20);
      tl.to(leftRef.current,  { opacity: 1, x: 0, filter: "blur(0px)", duration: 1.1 }, 0.44);
      tl.to(rightRef.current, { opacity: 1, x: 0, filter: "blur(0px)", duration: 1.0 }, 0.60);
        ...
        <div className="mt-4 flex gap-6 border-t border-white/[0.07] pt-3.5">
          {[
            ...
            <div key={k} className="flex flex-col gap-[5px]">
              <span className="font-mono text-[9px] tracking-[0.24em] text-white/28 uppercase">{k}</span>
              <span className="text-[13px] font-light text-white/76 tracking-[0.06em]">{v}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="relative z-[14] flex flex-col items-end pt-10">
      ...
      <div className="grid w-full max-w-[500px] gap-[10px]">
        {[
          { label:"ChatGPT",    desc:"思考・言語・解釈に強い対話OS",         meta:"LANGUAGE / STRUCTURE",    to:"/chatgpt",    glow:"cyan",    idx:0 },
          { label:"Gemini",     desc:"検索・画像・動画を横断する統合知性",   meta:"SEARCH / MULTIMODAL",     to:"/gemini",     glow:"sky",     idx:1 },
          { label:"Claude",     desc:"長文読解と安定推論に寄った静かな側近", meta:"LONG CONTEXT / STABILITY", to:"/claude",     glow:"violet",  idx:2 },
          { label:"DeepSeek",   desc:"演算、推論、コスパへ振り切った計算炉", meta:"COMPUTE / REASONING",     to:"/deepseek",   glow:"emerald", idx:3 },
          { label:"Midjourney", desc:"視覚世界を一枚へ圧縮する生成装置",    meta:"VISUAL / IMAGINATION",    to:"/midjourney", glow:"fuchsia", idx:4 },
        ].map((c) => <Category key={c.label} {...c} />)}
      </div>
    </div>
  );
}
...
function CoreScene() {
  return (
    <group>
      <Sparkles count={200} scale={[16, 12, 12]} size={2.3} speed={0.34} opacity={0.80} color="#9be7ff" />
      <Sparkles count={70}  scale={[10,  8,  8]} size={1.3} speed={0.20} opacity={0.46} color="#d5b7ff" />
...
function EnergyShell() {
  ...
  useFrame(({ clock }) => {
    ...
    if (wireRef.current) {
      ...
      wireRef.current.rotation.y = -t * 0.20;
    }
    if (haloRef.current) {
  ...
  return (
    <group position={[CX, CY, CZ]}>
      ...
      <mesh ref={inner1Ref} scale={0.72}>
        <torusKnotGeometry args={[1.18, 0.20, 240, 32, 2, 3]} />
      ...
      <mesh scale={1.36} rotation={[Math.PI / 3.2, 0.22, 0]}>
        ...
        <meshBasicMaterial color="#d5b7ff" transparent opacity={0.10} />
      </mesh>
...
function OrbitRings() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (g1Ref.current) { g1Ref.current.rotation.z = t * 0.055; g1Ref.current.rotation.y = t * 0.070; }
    if (g2Ref.current) { g2Ref.current.rotation.x = -t * 0.048; g2Ref.current.rotation.z = t * 0.038; }
  });
    ...
    <>
      ...
      <group ref={g2Ref} position={[CX, CY, CZ]}>
        <mesh rotation={[0.5, 0.9, 0.3]}>
          <torusGeometry args={[4.2, 0.010, 12, 160]} />
          <meshBasicMaterial color="#e5d7ff" transparent opacity={0.040} />
        </mesh>
      </group>
    </>
  );
...
function VerticalBeam() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (b1.current) { b1.current.scale.y = 1 + Math.sin(t * 1.55) * 0.058; b1.current.material.opacity = 0.16 + Math.sin(t * 1.9) * 0.040; }
    if (b2.current) { b2.current.scale.y = 1 + Math.cos(t * 1.2) * 0.044;  b2.current.material.opacity = 0.055 + Math.cos(t * 1.6) * 0.018; }
    if (b3.current) { b3.current.material.opacity = 0.030 + Math.sin(t * 0.85 + 1.2) * 0.012; }
  });
  ...
  return (
    <>
      ...
      <mesh ref={b3} position={[CX, CY, CZ]} rotation={[Math.PI/4, 0, Math.PI/4]}>
        ...
        <meshBasicMaterial color="#a8ffe0" transparent opacity={0.030} side={THREE.DoubleSide} />
      </mesh>
    </>
  );
}
    ...
    Array.from({ length: 14 }, (_, i) => {
      ...
      return {
        geom: new THREE.BufferGeometry().setFromPoints(
          new THREE.CatmullRomCurve3(pts).getPoints(42)
        ),
...
function PulseHalos() {
  ...
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    if (r1.current) {
      r1.current.scale.setScalar(1 + Math.sin(t * 1.85) * 0.050);
